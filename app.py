...
if uploaded_file:
    ...
    if "snx" in fname:
        unit_str = "m"
        y_axis_label = "Distance from Earth Core (m)"
        val_format = "{:,.4f}" # SNX는 숫자가 크므로 천 단위 콤마 추가
        data_type_name = "GEODETIC (공간 좌표)"
    elif "sp3" in fname or "clk" in fname:
        unit_str = "μs"
        y_axis_label = "Clock Offset (μs)"
        val_format = "{:.6f}"
        data_type_name = "GNSS SATELLITE (시간 오차)"
    else:
        unit_str = "Unit"
        y_axis_label = "Custom Measured Value"
        val_format = "{:.6f}"
        ...
        with st.spinner("🚀 데이터를 나노 단위로 해독 중입니다... (거대 파일은 1~2분 소요)"):
            ...
            # --- B. 만물 일반 데이터 파싱 (CSV, 엑셀) ---
                ...
                if numeric_cols:
                    ...
                    # 일반 파일의 경우 사용자가 단위를 입력할 수 있게 함
                    unit_str = st.text_input("이 데이터의 단위를 입력하세요 (예: nm, kg, %)", value="Unit")
                    y_axis_label = f"Measured Value ({unit_str})"
            ...
            if not full_df.empty:
                ...
                format_dict = {
                    'SI 기준': val_format, 
                    'K-Protocol': val_format, 
                    '보정율 (%)': '{:.4f}%', 
                    '남는변수': '{:,.6f}' if unit_str == 'm' else '{:.9f}'
                }
                ...
                # 라이브 메트릭에 동적 단위 반영
                c1.metric(f"SI 기준 ({unit_str})", f"{last['SI 기준']:,.4f}" if unit_str=='m' else f"{last['SI 기준']:.6f}")
                c2.metric(f"K-Protocol ({unit_str})", f"{last['K-Protocol']:,.4f}" if unit_str=='m' else f"{last['K-Protocol']:.6f}")
                c3.metric("보정율 (Sync)", f"{last['보정율 (%)']:.4f}%")
                c4.metric(f"남는변수 ({unit_str})", f"{last['남는변수']:,.4f}" if unit_str=='m' else f"{last['남는변수']:.9f}")

                fig = go.Figure()
                fig.add_trace(go.Scatter(y=sat_data['SI 기준'], name=f'SI Standard ({unit_str})', line=dict(color='red')))
                fig.add_trace(go.Scatter(y=sat_data['K-Protocol'], name=f'K-Protocol ({unit_str})', line=dict(color='blue')))
                fig.update_layout(yaxis_title=y_axis_label, hovermode="x unified", height=450)
