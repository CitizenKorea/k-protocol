...
# --- [D] 전리층/DCB 시각화 상세 (이어서) ---
            if not df_tec.empty:
                st.markdown("#### 🌌 전리층 총 전자수(TEC) 글로벌 히트맵")
                st.markdown("위도(Latitude)와 시간(Epoch)에 따른 전자 밀도 변화를 추적하여 우주 기상 상태를 분석합니다.")
                
                fig_tec = px.density_heatmap(
                    df_tec, x='Epoch', y='Latitude', z='TEC', 
                    histfunc='avg', color_continuous_scale='Turbo',
                    labels={'TEC': 'Total Electron Content (TECU)'},
                    title="Global Ionosphere Map (GIM) Analysis"
                )
                st.plotly_chart(fig_tec, use_container_width=True)
            
            if not df_inx.empty:
                st.markdown("#### 📡 위성별 하드웨어 코드 편향 (DCB)")
                st.markdown("위성 하드웨어에서 발생하는 신호 지연(Bias)과 신뢰도(RMS)를 비교 분석합니다.")
                fig_dcb = px.bar(
                    df_inx.sort_values('Bias'), x='PRN', y='Bias', color='RMS',
                    title="Satellite Differential Code Biases (PRN vs Bias)",
                    template="plotly_white",
...
# ==========================================
# 8. PDF Export (종합 무결성 리포트)
# ==========================================
# PDF 생성 버튼 로직 (기존 PDF 함수와 연동)
if uploaded_file is not None and (not df_spatial.empty or not df_temporal.empty or not df_obx.empty):
    st.divider()
    st.markdown("### 📄 K-PROTOCOL 분석 데이터 내보내기")
    
    # 리포트에 들어갈 핵심 요약 데이터 생성
    report_data = create_integrity_report(
        df_spatial, df_multi, df_temporal, 
        fname, fname, "2026-03-28", # 현재 에포크 예시
        r_sq if 'r_sq' in locals() else 99.9999
    )
    
    st.download_button(
        label=t['download_btn'], 
        data=report_data, 
        file_name=f"K_PROTOCOL_Full_Report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf", 
        mime="application/pdf", 
        type="primary"
    )

# ==========================================
# 9. Footer & Legal Notice
# ==========================================
st.divider()
col_f1, col_f2 = st.columns([2, 1])
with col_f1:
    st.caption("© 2026 K-PROTOCOL Omni Analysis Center. All rights reserved.")
    st.caption("본 시스템은 NASA CDDIS 및 IGS 정밀 궤도 데이터를 기반으로 시공간 기하학적 왜곡을 계산합니다.")
with col_f2:
    st.markdown("### ⚖️ Patent Pending")
    st.caption("K-PROTOCOL algorithm and related mathematical verifications are strictly patent pending.")

# 최종 실행 확인용 로그
if uploaded_file:
    st.toast(f"파일 분석 완료: {fname}", icon="✅")
